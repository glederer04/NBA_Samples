"""Load missing NBA player box scores in controlled batches."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Load missing NBA player box scores.")
    parser.add_argument(
        "--team",
...
def main() -> None:
    """Run resumable player box-score ingestion."""

    args = parse_args()

    pending_game_ids = get_pending_game_ids(
    ...
    if not pending_game_ids:
        print("No missing player box scores matched the request.")
        return
    ...
    with track_pipeline_run(
        pipeline_name="ingest_player_box_score_batch",
        database_path=DATABASE_PATH,
    ) as pipeline_run:
        ...
        if result.failed_game_ids:
            ...
            raise RuntimeError(f"Box-score ingestion failed for: {failed_games}")
    ...
    print(f"Player records loaded: {result.loaded_rows}")

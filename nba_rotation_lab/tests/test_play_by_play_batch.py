"""Tests for batch play-by-play ingestion and coverage."""
...
def seed_pending_play_by_play_data(
    database_path: Path,
) -> None:
    """Insert games with different play-by-play states."""
...
def test_get_pending_play_by_play_game_ids(
    ...
    seed_pending_play_by_play_data(database_path)

    all_pending_games = get_pending_play_by_play_game_ids(
        database_path=database_path,
    )
    knicks_pending_games = get_pending_play_by_play_game_ids(
        database_path=database_path,
...
def test_ingest_play_by_play_game_ids_retries(
    ...
    """Transient play-by-play failures should retry successfully."""
    ...
    def fake_loader(
        ...
        if game_id == "001" and attempts[game_id] == 1:
            raise TimeoutError("Temporary play-by-play timeout")
...
def test_play_by_play_coverage_summary(
    tmp_path: Path,
) -> None:
    """Matching final scores should produce complete coverage."""
